...
def build_nfs_scenarios(n_s: int, x: np.array, y_scaler, flow, conditioner_args, max:int=1, gpu:bool=True, tag:str= 'load', non_null_indexes:list=[]):
    """
    ...
    for i in range(n_days):
        ...
        scenarios.append(predictions.transpose()) # list of arrays of shape (24, n_s)
        end = timer()
        time_tot += end - start
...
def fit_NF(nb_epoch: int, x_LS: np.array, y_LS: np.array, x_VS: np.array, y_VS: np.array, x_TEST: np.array, y_TEST: np.array, flow, opt, batch_size:int=100, wdb:bool=False, gpu:bool=True):
    ...
    for epoch in tqdm(range(nb_epoch)):
        ...
        i = 0
        loss_batch = 0
        batch_list = [i for i in range(batch_size, batch_size * y_VS.shape[0] // batch_size, batch_size)]
        for y_batch, x_batch in zip(np.split(y_VS, batch_list), np.split(x_VS, batch_list)):
            ll, z = flow.compute_ll(x=torch.tensor(y_batch).to(device).float(), context=torch.tensor(x_batch).to(device).float())
        ...
        loss_vs = loss_batch / i

        i = 0
        loss_batch = 0
        batch_list = [i for i in range(batch_size, batch_size * y_TEST.shape[0] // batch_size, batch_size)]
        for y_batch, x_batch in zip(np.split(y_TEST, batch_list), np.split(x_TEST, batch_list)):
            ll, z = flow.compute_ll(x=torch.tensor(y_batch).to(device).float(), context=torch.tensor(x_batch).to(device).float())
            loss_batch += -ll.mean().item()
            i += 1
        # VS loss
        ...
        if not math.isnan(loss_vs) and loss_vs <= ll_VS_min:
            ...
            torch.save(best_flow, r'C:\Thesis\scripts\normalizing-flows\export.pt')
