...
import numpy as np
from energyclustering.cobras.cluster import Cluster

from energyclustering.cobras.clustering import Clustering


class COBRAS(abc.ABC):
    def __init__(self, data, querier, max_questions, train_indices=None, store_intermediate_results=True, log_path = None, seed = None):
        """
        COBRAS clustering
    ...
    def cluster(self):
        ...
        # initial_k = yield from self.determine_split_level(initial_superinstance,
        ...
        superinstances = self.split_superinstance(initial_superinstance, initial_k, self.seed)
        ...
        yield from self.merge_containing_clusters(copy.deepcopy(self.clustering.construct_cluster_labeling()))
        ...
        while len(self.ml) + len(self.cl) < self.max_questions:
            ...
            # split_level = yield from self.determine_split_level(to_split, clustering_to_store)
            split_level = 3

            # split the chosen super-instance
            new_super_instances = self.split_superinstance(to_split, split_level, self.seed)
            ...
            fully_merged = yield from self.merge_containing_clusters(clustering_to_store)
            ...
            if fully_merged:
                ...
                if self.log_path is not None:
                    ...
                    path = self.log_path/'clusters'/f'clustering{self.cluster_counter}.txt'
                    super_instance_path = self.log_path/'superinstances'/f'superinstances{self.cluster_counter}.txt'
                    self.cluster_counter += 1
                    path.parent.mkdir(parents = True, exist_ok = True)
                    super_instance_path.parent.mkdir(parents = True, exist_ok = True)
                    np.savetxt(path, self.clustering.construct_cluster_labeling())
                    np.savetxt(super_instance_path, self.clustering.get_all_superinstance_representatives())
    ...
    def determine_split_level(self, superinstance, clustering_to_store):
        ...
        while not must_link_found and len(self.ml) + len(self.cl) < self.max_questions:
            ...
            if len(si.indices) == 2:
                ...
                new_si = self.split_superinstance(si, 2, self.seed)
            ...
            query_result = yield from self.querier.query_points(pt1, pt2)
            if query_result:
