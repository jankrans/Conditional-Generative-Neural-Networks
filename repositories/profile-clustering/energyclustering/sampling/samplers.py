...
from energyclustering.sampling.util import cluster_probabilities_to_sample_probabilities


class BaseSampler(metaclass=abc.ABCMeta):
    """
    Abstract class for base samplers, this means that based on some info these classes allow you to sample some data
    ...
    def clean(self):
        """
            Method that deletes unnecessary data after fitting.

            This is to optimize the experiments
        """
        raise NotImplementedError()
...
class ClusteringBaseSampler(BaseSampler):
    """
    Abstract class for samplers that use a clustering (some more complex sampler classes need direct access to the clustering
    """
    ...
    def clustering_dict(self):
        ...
        """
        return {index: df.index for index, df in self.clustering.groupby(self.clustering)}
    ...
    def generate_samples(self, info, nb_samples):
        ...
        for index in range(info.shape[0]):
            picked_clusters = np.random.choice(cluster_probabilities.shape[1], size=nb_samples, replace=True,
            ...
            for cluster_idx, count in zip(cluster_idxs, counts):
                samples.append(cluster_dict[cluster_idx].sample(count, replace=True).index.to_numpy())
            samples_per_info_row.append(np.concatenate(samples, axis=0))
        return samples_per_info_row


class MetadataSampler(ClusteringBaseSampler):
    """
    ...
    def get_cluster_probabilities(self, test_info):
        ...
        # e.g. cluster_probabilities[i,j] = probability that cluster j is chosen for sample i (in this case only 0 and 1)
        cluster_probabilities = np.zeros((test_info.shape[0], self.cluster_centroids.shape[0]))
    ...
    def get_sampling_probabilities(self, test_info):
        ...
        for cluster_idx in non_zero_clusters:
            ...
            probability_series[cluster_idx] = pd.Series(np.full(len(profiles), fill_value=1 / len(profiles)),
                                                        index=profiles)

        sample_probs_per_test_info = []
        for cluster_idx, test_info_entry in zip(assigned_clusters, test_info.index):
            sample_probs_per_test_info.append(probability_series[cluster_idx].rename(test_info_entry))

        return sample_probs_per_test_info

    def old_get_sampling_probabilities(self, test_info):
        test_info = self.preprocess_info_transform(test_info)
        ...
        # sample uniformly from each assigned clsuter
        sample_probabilities = cluster_probabilities_to_sample_probabilities(cluster_probabilities, self.clustering)

        # return the result as a dataframe
        sampling_df = pd.DataFrame(sample_probabilities, index=test_info.index, columns=self.clustering.index)
        return list(item for _, item in sampling_df.iterrows())
...
class ConsumptionDataSampler(ClusteringBaseSampler):
    """
    ...
    def __init__(self, classifier, clusterer, info_preprocessing=None, deterministic=False, fillna=True):
        super().__init__(info_preprocessing)
    ...
    def fit(self, info, consumption_data):
        ...
        self.clusterer.fit(consumption_data)
        self.clustering = pd.Series(self.clusterer.labels_, index=consumption_data.index)
        self.clustering_lookup_dict = {label: df for label, df in self.clustering.groupby(self.clustering)}
    ...
    def get_sampling_probabilities(self, test_info):
        ...
        sample_probabilities = cluster_probabilities_to_sample_probabilities(cluster_probabilities, self.clustering)
        sample_df = pd.DataFrame(sample_probabilities, index=test_info.index, columns=self.clustering.index)
    ...
    def generate_samples(self, info, nb_samples):
        ...
        for cluster_probs in cluster_probabilities:
            picked_clusters = np.random.choice(np.arange(0, cluster_probs.shape[0]), nb_samples, replace=True, p=cluster_probs)
            ...
            for cluster_idx, count in zip(cluster_idxs, cluster_count):
                samples_from_cluster = self.clustering_lookup_dict[cluster_idx].sample(count, replace = True).index.to_list()
...
class EnergyvilleDaySelectionBaseline(BaseSampler):
    ...
    def fit(self, info, consumption_data):
        assert all(required_col in info.columns for required_col in
                   self.REQUIRED_COLUMNS), 'FeelsLikeC should be in the info df'
    ...
    def _filter_based_on_FeelsLikeC(self, df, temp):
        ...
        matching_t_days = df.query(f'feelsLikeC >= {min_t_threshold} and feelsLikeC <= {max_t_threshold}')
    ...
    def get_sampling_probabilities(self, test_info):
        ...
        for (testMeterID, test_date), query_series in test_info.iterrows():
            ...
            matching_days = self._filter_based_on_FeelsLikeC(training_info, float(query_series.feelsLikeC))

            # if no similar weather days just use same weekday in same month
            if matching_days.shape[0] == 0:
                # print(query_series)
                # print(self.info.shape)
                matching_days = self._filter_based_on_daytype(training_info, query_series.dayOfWeek, query_series.month)
            ...
            sampling_probs = pd.Series(np.full((matching_days.shape[0],), 1 / matching_days.shape[0]),
                                       index=matching_days.index).rename((testMeterID, test_date))
            all_sampling_probs.append(sampling_probs)
        return all_sampling_probs


class RandomSamplerBaseline(BaseSampler):
    ...
    def old_get_sampling_probabilities(self, test_info):
        daily_probabilities = self.get_sampling_probabilities(test_info)
        prob_matrix = (
            pd.concat(daily_probabilities, axis=1, keys=test_info.index)
            .reindex(self.consumption_data.index, fill_value=0)
            .T
        )
        return prob_matrix
    ...
    def get_sampling_probabilities(self, test_info):
        result = []
        samples_to_take = self.consumption_data.shape[0] if self.n_samples is None else min(
            self.consumption_data.shape[0], self.n_samples)
        for test_day in test_info.index:
            sample = self.consumption_data.sample(samples_to_take, random_state=self.bit_generator)
