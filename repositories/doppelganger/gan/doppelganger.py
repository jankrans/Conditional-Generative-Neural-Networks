            ...
            if global_id - 1 > restore_global_id:
                if ((epoch_id + 1) % self.epoch_checkpoint_freq == 0 or
                    ...
                    with open(self.time_path, "a") as f:
                        ...
                        self.g_loss
                        f.write(f"{epoch_id}")
     #;{self.attr_d_loss_fake.eval()};{self.attr_d_loss_real.eval()};{self.attr_d_loss_gp.eval()};{self.attr_d_loss.eval()};{self.d_loss_fake.eval()};{self.d_loss_real.eval()};{self.d_loss_gp.eval()};{self.d_loss.eval()};{self.g_loss.eval()};{self.g_loss_attr_d.eval()};{self.g_loss_d.eval()}")
